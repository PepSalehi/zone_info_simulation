...
class Operator():
    def __init__(self, report, which_day_numerical=2, name="Uber", BONUS=BONUS, 
                 SURGE_MULTIPLIER=SURGE_MULTIPLIER):
        self.name = name 
        self.demand_fare_stats_prior = pd.read_csv( "./Data/df_hourly_stats_over_days.csv")
        self.demand_fare_stats_of_the_day = pd.read_csv( "./Data/df_hourly_stats.csv").query('Day=={d}'.format(d=which_day_numerical))
        self.live_data = None 
        self.revenues =[]
...
#        df = df.assign(match_prob=df['total_pickup']/df.total_pickup.sum())  # pax/min just the default 
    ...
    def false_zonal_info_over_t(self, t):
        ...
        df =  self.demand_fare_stats_of_the_day.query("Hour == {hour}".format(hour=t))
        #
        df.loc[df['Origin'].isin(zone_ids),'total_pickup']  = df[df['Origin'].isin(zone_ids) ]['total_pickup']*False_mult
        df.loc[~df['Origin'].isin(zone_ids),'total_pickup']  = df[~df['Origin'].isin(zone_ids) ]['total_pickup']/False_mult
        df = df.assign(surge=1)
        df = df.assign(bonus=0) 
        df = df.assign(match_prob=df['total_pickup']/60)  # pax/min just the default 
...
#        if self.live_data is None:
    ...
    def update_zone_policy(self, t, zones, WARMUP_PHASE ):
        ...
        if t % POLICY_UPDATE_INTERVAL == 0:
            for z in zones: 
                ratio  = len(z.demand)/(len(z.idle_vehicles) + len(z.incoming_vehicles) + 1)
