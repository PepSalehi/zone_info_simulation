...
class Model():
    ...
    def __init__(self, zone_ids, daily_demand, WARMUP_TIME_HOUR, ANALYSIS_TIME_HOUR, 
        ...
        if self.PRO_SHARE >0:
            m= pickle.load(open("outputs/model for fleet size {f} surge {s}fdemand 0.0perc_k 1pro_s 0.p".format(f=self.FLEET_SIZE, s= _s), 'rb'))
        ...
        self.fleet_deceived_size= int(self.percent_false_demand  * self.non_av_fleet_size )
        self.percentage_know_fare = percentage_know_fare
        self.fleet_DONT_know_fare = int((1-self.percentage_know_fare ) * self.non_av_fleet_size )
        
        print("AV_Share is ", self.AV_SHARE)
        print ("av fleet size is ", self.fleet_AV)
        self._create_vehicles()
        
        print("the number of AV vehicles are ", len([v for v in self.vehilcs if v.is_AV ]))
...
if __name__ == "__main__":
    ...
    # # m = Model([1,2])
    # # print(ZONE_IDS)
    # m = Model(ZONE_IDS, DEMAND_SOURCE, WARMUP_TIME_HOUR, ANALYSIS_TIME_HOUR)
    # #m.set_analysis_time(WARMUP_TIME_HOUR)
    
    # # z = m.zones[0]
    # # z.set_demand_rate_per_t(7) # 8am 
